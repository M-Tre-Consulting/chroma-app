...
import { MobileAndroidApp } from "./MobileAndroidApp";

function App() {
  const platform = usePlatform();
  const [tab, setTab] = useState<"palettes" | "tokens" | "export">("palettes");
  const { palettes, activePaletteId } = usePaletteStore();
  const activePalette = palettes.find((p) => p.id === activePaletteId);

  console.log('detected platform: ', platform);

  if (platform === "android") return <MobileAndroidApp />;
      ...
      >
        ...
        <div className="px-2 py-1.5">
          ...
          >
            ...
            />
            {(["palettes", "tokens", "export"] as const).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          >
            ...
            {activePalette?.colours.map((c) => (
              <ColourCard key={c.id} colour={c} paletteId={activePalette.id} />
            ))}
          </div>
        </div>
      </main>
